...
std::vector< std::pair< int, int> > get_terms()  {
    ...
    if (polynomial[last - 1].second != 0)  {
      //error = true;  // x^0 = 1 from first version (2018)
      // In this version I will add a constant term of 0 (zero)
      // This amounts to factoring out variable so there will be a factor of 0
      std::pair<int, int> p(0, 0);  // 0*x^0 = 0*1 = 0
      polynomial.push_back(p);
      last += 1;  // Now there is one more term
    }
    if (polynomial[last - 1].first == 0)  {
      // error = true;   // must have constant term not zero [OLD VERSION]
      zero_factor = true;
      last -= 1;  // removing self from list of terms
      polynomial.pop_back();
      // Now we have to adjust the exponents, substracting 1 from each
      for (int i = 0; i < last; i++)  {
